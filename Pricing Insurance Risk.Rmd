...
```{r Ex1}
accident <- sum(total_data$Accident == 'Yes', na.rm = TRUE)/nrow(total_data)
accident
```
...
Truck has the highest probability of having an accident. An individual with a better credit history seem to be better driver.
...
gam1_acc <- confusionMatrix(factor(ifelse(lm1_pred>0.5, 'Yes', 'No')), test_data$Accident, positive = 'Yes')
...
nn1 has the highest sensitivity and lm1 has the highest specificity. 
...
```{r Ex8}
library(caret)
lift_chart <- caret::lift(test_data$Accident~lm1_pred+gam1_pred+nn1_pred, class = 'Yes', cuts = 200)
...
Take in offer about 45% of the drivers. 
...
AreaE has the highest expected claim amount per Accident
